 ...
 *     author: YanWen
 *     time  : 2020/01/13
 *     desc  : SP 相关工具类，lib中已经有了SPUtils类，这里命名为SP01Utils;
 *     warning  : 部分功能需要先到Application类里实现初始化---- YwUtils.init(application);
 * </pre>
 */
@SuppressLint("ApplySharedPref")
public final class SP01Utils {

    private static SimpleArrayMap<String, SP01Utils> SP_UTILS_MAP = new SimpleArrayMap<>();
     ...
     * @return {@link SPUtils}
     */
    public static SP01Utils getInstance() {
        return getInstance("");
    }
     ...
     * @return {@link SPUtils}
     */
    public static SP01Utils getInstance(String spName) {
        if (isSpace(spName)) spName = "spUtils";
        SP01Utils spUtils = SP_UTILS_MAP.get(spName);
        if (spUtils == null) {
            spUtils = new SP01Utils(spName);
            SP_UTILS_MAP.put(spName, spUtils);
        }
        return spUtils;
    }

    private SP01Utils(final String spName) {
        sp = YwUtils.getApp().getSharedPreferences(spName, Context.MODE_PRIVATE);
    }
